...
public class PlayerPropBinocularsState : PlayerBaseState
{
    readonly int FreeLookBlendTreeHash = Animator.StringToHash("FreeLookBlendTree");

    public PlayerPropBinocularsState(PlayerStateMachine stateMachine) : base(stateMachine)
    {
    }
    ...
    void OnAim()
    {
        _stateMachine.SwitchState(new PlayerPropBinocularsZoomState(_stateMachine));
    }
}