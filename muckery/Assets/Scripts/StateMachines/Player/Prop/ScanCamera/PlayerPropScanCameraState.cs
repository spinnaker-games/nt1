...
public class PlayerPropScanCameraState : PlayerBaseState
{
    readonly int FreeLookBlendTreeHash = Animator.StringToHash("FreeLookBlendTree");

    public PlayerPropScanCameraState(PlayerStateMachine stateMachine) : base(stateMachine)
    {
    }
    ...
    void OnAim()
    {
        _stateMachine.SwitchState(new PlayerPropScanCameraZoomState(_stateMachine));
    }
}